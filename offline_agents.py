...
class A2Cmp(A2C):
    def __init__(self, beta, v_fn):
        super(A2Cmp, self).__init__(beta, v_fn)
        self.name = "A2C"
    ...
    def get_phi(self, trajectory, critic):
        ...
        next_states = torch.stack(trajectory["next_states"]).to(device)
        masks = torch.stack(trajectory["masks"]).to(device)
        values = critic.get_max_value(states).squeeze(-1)
        if bootstrap: next_values = critic.get_max_value(next_states)
        else: next_values = torch.zeros(rewards.size(0)).to(device)
        returns = torch.Tensor(rewards.size()).to(device)
        deltas = torch.Tensor(rewards.size()).to(device)
        advantages = torch.Tensor(rewards.size()).to(device)
        prev_return = 0
        prev_value = 0
        prev_advantage = 0
        for i in reversed(range(rewards.size(0))):
    ...
    def update(self, pol_opt, q_opt, v_opt, trajectory):
        ...
        for i in range(iters):
            ...
            phi = advantages / returns.std()
            values = self.v_fn(states)
            v_fn_loss = torch.mean(torch.cat([(v.squeeze(-1) - returns.detach()) ** 2 for v in values], dim=-1))

            lp_p = self.beta.get_log_prob(states, actions)
            ratio = torch.exp(lp_p - log_probs.detach())
            pol_loss = -torch.mean(ratio.squeeze(-1) * phi.detach())
            loss = pol_loss + v_fn_loss
...
class PPOmp(A2Cmp):
    ...
    """
    def __init__(self, beta, v_fn):
        super(PPOmp, self).__init__(beta, v_fn)
        self.name = "PPO"
    ...
    def update(self, pol_opt, q_opt, v_opt, trajectory):
        ...
        for i in range(iters):
            ...
            lp_p = self.beta.get_log_prob(states, actions)
            ratio = torch.exp(lp_p - log_probs.detach()).squeeze(-1)
            clipped_objective = torch.min(ratio * phi, torch.clamp(ratio, 1 - eps, 1 + eps) * phi)
            pol_loss = -torch.mean(clipped_objective)
            loss = pol_loss + v_fn_loss
